...
var _mindPool = null;  // 念头池 + 12 维欲望 + intent（/api/mind/flash-pool）
var _mindCounts = null; // { feels:{active,fading,sleeping}, memories:{...} } 给筛选按钮显示计数
...
function _loadMindData() {
  // 念头池跟 state 一起拉：地层页的「最想做的一件事」和「方向」都要它，
  // 拿不到就退回三桶能量，别整页空着。
  _mindApi('GET', '/api/mind/flash-pool').then(function(p) {
    if (p && p.ok) {
      _mindPool = p;
      if (_mindPage === 'layer' && _mindData) { _mindData.layer = _buildLayerFromState(_mindData.layer.state); _renderLayerPage(); }
    }
  }).catch(function() {});
  _mindApi('GET', '/api/mind/state').then(function(res) {
    if (res && res.ok) {
      _mindCounts = { feels: res.feels || {}, memories: res.memories || {} };
...
function _buildLayerFromState(state) {
  ...
  // 「最想做的一件事」= 后端 pickIntent() 挑出来的那一维，不是写死的文案。
  // ⚠️ 这里以前硬编码成「心里压着事的时候，我会做梦」+ 梦的能量条，
  //    于是不管 12 维欲望怎么变，面板永远只说"梦"。
  var desire, directions;
  var intent = _mindPool && _mindPool.intent;
  if (intent) {
    desire = { text: intent.line || intent.label, action: intent.action || '去做', progress: intent.level || 0 };
  } else {
    desire = { text: '还没有哪一维压过别的。再多聊几句吧。', action: '等着', progress: 0 };
  }
  // 方向：12 维欲望缺口，按 level 从高到低。拿不到池子才退回三桶能量。
  var levels = _mindPool && _mindPool.levels;
  if (levels) {
    directions = Object.keys(levels).map(function(dk) {
      return { key: levels[dk].label || dk, drive: dk, value: levels[dk].level || 0 };
...
function _renderFeelPage() {
  ...
  // Filter pills —— 带计数。空桶显示 0，才看得出是"还没淡到那儿"而不是"面板坏了"。
  var c = (_mindCounts && _mindCounts.feels) || {};
  html += '<div class="mind-feel-filter">';
  html += _feelFilterBtn('active', '活跃', c.active);
  html += _feelFilterBtn('fading', '淡了', c.fading);
  html += _feelFilterBtn('sleeping', '沉睡', c.sleeping);
  html += '</div>';
...
function _feelFilterBtn(key, label, n) {
  var txt = label + (typeof n === 'number' ? ' ' + n : '');
  return '<button class="mind-feel-filter-btn ' + (_mindFeelFilter === key ? 'active' : '') +
    '" onclick="_setFeelFilter(\'' + key + '\')">' + txt + '</button>';
}
