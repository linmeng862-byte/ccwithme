...
function _showBookshelf() {
  ...
  panel.innerHTML =
    // header — 细导航栏，居中标题
    '<div style="display:flex;align-items:center;justify-content:space-between;padding:calc(env(safe-area-inset-top) + 12px) 20px 8px;flex:none">' +
...
function _showAddBookSheet() {
  ...
  var html = '' +
    ...
    // Header
    '<div style="flex:none;padding:12px 24px 6px"><div style="font:600 26px/1.2 var(--font-serif);color:#1C1B19;letter-spacing:-.01em">Add a Book</div></div>' +
    // 文件信息
  ...
  try {
    ...
    if (_addBookFile.length) {
      ...
      for (var i = 0; i < _addBookFile.length; i++) {
        ...
        await fetch('/api/reading/upload', { method: 'POST', headers: { 'Authorization': 'Bearer ' + (state.token || '') }, body: fd });
        count++;
      }
...
async function _loadAnnotationLog() {
  ...
  try {
    ...
    var allAnnotations = await resp.json();
    if (_annoSearchTerm) {
    ...
    allAnnotations.forEach(function(a) {
      var key = a.book_id;
      if (!bookMap[key]) bookMap[key] = { id: a.book_id, title: a.book_title || 'Unknown', author: a.book_author || '', highlights: 0, latest: '', items: [] };
    ...
    // 统计头部
    var statsHtml = '<div style="text-align:center;padding:0 24px 20px"><div style="font:400 13px/1.4 var(--font-sans);color:var(--books-muted)">' + totalHighlights + ' Highlights · ' + totalBooks + ' Book' + (totalBooks !== 1 ? 's' : '') + '</div></div>';
    ...
    var cardsHtml = books.map(function(b) {
      ...
      return '<div onclick="_showBookAnnotations(\'' + b.id + '\')" style="margin-bottom:16px;padding:18px 18px 18px 20px;background:#fff;border-radius:20px;cursor:pointer;display:flex;align-items:center;gap:16px" oncontextmenu="event.preventDefault();_showBookDelete(\'' + b.id + '\',\'' + _escJS(b.title) + '\')" ontouchstart="_bookTouchStart(event,\'' + b.id + '\',\'' + _escJS(b.title) + '\')" ontouchend="_bookTouchEnd(event)" ontouchmove="_bookTouchMove(event)">' +
        ...
        '<div style="font:400 13px/1.4 var(--font-sans);color:var(--books-faint);margin-top:2px">' + b.highlights + ' highlight' + (b.highlights !== 1 ? 's' : '') + (latestRel ? ' · ' + latestRel : '') + '</div>' +
        '</div>' +
        // 右：箭头
...
function _showBookAnnotations(bookId) {
  ...
  list.innerHTML =
    ...
    '</div>' +
    '<div style="font:400 13px var(--font-sans);color:var(--books-faint);flex:none">' + book.highlights + ' highlight' + (book.highlights !== 1 ? 's' : '') + '</div>' +
    '</div>' +
    // 批注卡片列表
    book.items.map(function(a) {
      var color = _hlColors[(a.who || 'y').charAt(0)] || _hlColors.y;
      ...
      'ontouchend="_annoPressEnd(event)" ontouchmove="_annoPressEnd(event)" onmouseup="_annoPressEnd(event)" onmouseleave="_annoPressEnd(event)">' +
        ...
        '</div>' +
        '<div style="margin:0 0 ' + (a.note || lastReply ? '14' : '0') + 'px;font:italic 16px/1.7 var(--font-serif);color:rgba(0,0,0,.55)"><mark style="background:' + color + ';border-radius:3px;padding:1px 0">' + escHtml((a.anchor || '').slice(0, 300)) + '</mark></div>' +
        ...
        (lastReply ? '<div style="padding-top:' + (a.note ? '6' : '0') + 'px;border-top:' + (a.note ? '1px solid rgba(0,0,0,.04)' : 'none') + '">' +
          '<span style="font-weight:600;font-size:12px;color:var(--accent,#C89664)">' + (lastReply.who === 'ai' ? 'Claude' : '粥粥') + '</span>' +
...
function _renderBodyWithMarks(content, annotations) {
  ...
  for (var i = 0; i < sorted.length; i++) {
    ...
    // 划线颜色：who 字段存 y/g/b/r，默认 y
    var c = (a.who || 'y').charAt(0);
    var color = _hlColors[c] || _hlColors.y;
...
async function _renderReader() {
  ...
  panel.innerHTML =
    // 顶栏
    '<header style="display:flex;align-items:center;gap:10px;padding:calc(env(safe-area-inset-top) + 10px) 14px 8px;flex:none">' +
    ...
    '<div id="readerBody" style="font:400 17px/1.9 var(--font-serif);color:#2E2A26">' + bodyHtml + '</div>' +
    '</div></div>' +
  ...
  $('readerBack').onclick = function() { _showBookshelf(); _saveProgress(); };
...
// === 批注加载 + 轮询 ===
var _annotations = [], _pollTimer = null, _lastPollTime = 0;
...
function _startAnnotationPolling() {
  ...
  _pollTimer = setInterval(function() {
    _loadAnnotations(false).then(function() {
      var body = $('readerBody'); if (!body || !_annotations.length) return;
      var ch = _readerBook && _readerBook.chapters ? _readerBook.chapters[_readerChapterIdx] : null;
      if (!ch) return;
      body.innerHTML = _renderBodyWithMarks(ch.content, _annotations);
...
var _hlColors = {
  y: 'rgba(255,242,148,.40)',
  p: 'rgba(250,192,212,.40)',
  g: 'rgba(188,228,188,.40)',
  b: 'rgba(182,214,244,.40)'
};
// 工具栏圆点用（更实色）
var _hlColorsSolid = {
  y: 'rgba(248,235,150,.65)',
  p: 'rgba(240,192,205,.65)',
  g: 'rgba(188,224,188,.65)',
  b: 'rgba(180,212,242,.65)'
};
var _hlColorsSolid = {
  y: '#F5D850',
  o: '#F0B860',
...
function _showSelBar(sel, text) {
  ...
  var dot = function(c) {
    var core = { y: '#E8D050', p: '#E090A8', g: '#80B880', b: '#78A8D8' };
    return '<span style="width:26px;height:26px;border-radius:50%;background:' + _hlColorsSolid[c] + ';display:grid;place-items:center;cursor:pointer;flex:none;border:1.5px solid rgba(0,0,0,.08)" title="' + c + '" onclick="(function(){_doHighlight(\'' + c + '\',\'' + _escJS(t) + '\');_removeSelBar()})()"><span style="width:10px;height:10px;border-radius:50%;background:' + core[c] + '"></span></span>';
  };
  bar.innerHTML =
...
function _openNoteEditor(text) {
  ...
  var colorDots = colorKeys.map(function(k) {
    var sel = k === _noteEditorColor;
    var core = { y: '#E8D050', p: '#E090A8', g: '#80B880', b: '#78A8D8' };
    return '<span data-nc="' + k + '" style="width:30px;height:30px;border-radius:50%;background:' + _hlColorsSolid[k] + ';display:grid;place-items:center;cursor:pointer;flex:none;border:1.5px solid rgba(0,0,0,.08);' + (sel ? 'box-shadow:0 0 0 3px rgba(0,0,0,.12);' : '') + '"><span style="width:10px;height:10px;border-radius:50%;background:' + core[k] + '"></span></span>';
  }).join('');
  ...
  var html = '' +
    ...
    // 引文
    '<blockquote style="margin:0 0 20px;padding:14px 18px;background:rgba(0,0,0,.025);border-radius:16px;font:italic 16px/1.6 var(--font-serif);color:rgba(0,0,0,.5)">' + escHtml(text.slice(0, 300)) + '</blockquote>' +
    // 颜色选择
    ...
    document.querySelectorAll('#noteEditorSheet [data-nc]').forEach(function(dot) {
      dot.addEventListener('click', function() {
        ...
        document.querySelectorAll('#noteEditorSheet [data-nc]').forEach(function(d) {
          d.style.boxShadow = d.getAttribute('data-nc') === _noteEditorColor ? '0 0 0 3px rgba(0,0,0,.12)' : '';
        });
      });
    });
...
function _doSaveNote() {
  ...
  _saveAnnotationWithColor(window._noteEditorText, note, _noteEditorColor);
  _closeNoteEditor();
}
...
async function _refreshAnnotations() {
  ...
  var ch = _readerBook && _readerBook.chapters ? _readerBook.chapters[_readerChapterIdx] : null;
  if (!ch) return;
  body.innerHTML = _annotations.length > 0
    ? _renderBodyWithMarks(ch.content, _annotations)
    : _fmtParagraphs(ch.content);
  // 重新绑定 mark 点击
  body.querySelectorAll('mark.anno-mark').forEach(function(m) {
...
function _showAnnotationSheet(annotation) {
  ...
  var curColor = (annotation.who || 'y').charAt(0);
  ...
  var whoLabel = (annotation.who && annotation.who.length === 1) ? '粥粥' : (annotation.who || '粥粥');

  var colorKeys = ['y','p','g','b'];
  var colorDots = colorKeys.map(function(k) {
    var sel = k === curColor;
    var core = { y: '#E8D050', p: '#E090A8', g: '#80B880', b: '#78A8D8' };
    return '<span data-c="' + k + '" style="width:28px;height:28px;border-radius:50%;background:' + _hlColorsSolid[k] + ';display:grid;place-items:center;cursor:pointer;flex:none;border:1.5px solid rgba(0,0,0,.08);' + (sel ? 'box-shadow:0 0 0 3px rgba(0,0,0,.12);' : '') + '"><span style="width:10px;height:10px;border-radius:50%;background:' + core[k] + '"></span></span>';
  }).join('');
  ...
  var html = '' +
    ...
    '<div id="annotationSheet" style="position:fixed;inset:auto 0 0;z-index:86;height:' + _sheetHeight + 'vh;background:rgba(248,246,242,.88);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border-radius:24px 24px 0 0;transform:translateY(100%);transition:transform .45s cubic-bezier(.22,.61,.36,1);overflow:hidden;display:flex;flex-direction:column;box-shadow:0 -2px 24px rgba(0,0,0,.08)">' +
    // 拖拽把手 + 关闭
    ...
    // 引文：灰底卡片 + 荧光笔 mark v2
    '<div style="margin:0 0 16px;padding:16px 18px;background:rgba(0,0,0,.045);border-radius:16px;font:italic 16px/1.75 var(--font-serif);color:rgba(0,0,0,.45)"><mark style="background:' + color + ';border-radius:4px;padding:1px 2px">' + escHtml((annotation.anchor || '').slice(0, 300)) + '</mark></div>' +
    // 批注：同色竖线 + 批注人 · 时间 + 内容
    (annotation.note ? '<div style="display:flex;gap:12px;margin-bottom:20px">' +
      '<div style="width:4px;border-radius:2px;background:' + color + ';flex:none"></div>' +
      '<div style="flex:1;min-width:0">' +
      '<div style="font-weight:600;font-size:13px;color:var(--books-text);margin-bottom:2px">' + whoLabel + (ts ? ' <span style="font-weight:400;font-size:11px;color:var(--books-muted)">· ' + ts + '</span>' : '') + '</div>' +
    ...
    replies.map(function(r) {
      ...
      return '<div style="display:flex;gap:10px;margin-bottom:16px;position:relative">' +
        '<div style="width:4px;border-radius:2px;background:rgba(137,174,197,.35);flex:none"></div>' +
        '<div style="flex:1;min-width:0">' +
        '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:3px">' +
        '<span style="font-weight:600;font-size:13px;color:var(--books-text)">' + rWho + '</span>' +
...
async function _updateAnnotationColor(annotation, color) {
  if (!annotation || !currentReadingBookId) return;
  try {
    await fetch('/api/reading/books/' + currentReadingBookId + '/annotations/' + annotation.id, {
...
async function _postReply(annotation, input) {
  ...
  if (!text || !annotation || !currentReadingBookId) return;
  try {
    var resp = await fetch('/api/reading/books/' + currentReadingBookId + '/annotations/' + annotation.id + '/replies', {
      method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + (state.token || '') },
...
async function _deleteAnnotation(annotation) {
  ...
  try {
    var resp = await fetch('/api/reading/books/' + currentReadingBookId + '/annotations/' + annotation.id, {
      method: 'DELETE', headers: { 'Authorization': 'Bearer ' + (state.token || '') }
    });
    if (resp.ok) {
      toast('已删除');
  ...
  _annoPressTimer = setTimeout(function() {
    ...
    (async function() {
      try {
        ...
        if (resp.ok) { toast('已删除'); if (_currentAnnoBookId) { _showBookAnnotations(_currentAnnoBookId); } else { _loadAnnotationLog(); } }
      } catch(e) {}
    })();
  }, 600);
}
