...
9. **禁止删除 VPS 上任何东西。** 文件、目录、进程、数据库、容器、快照一律不删；要腾地方就改名或移进 `trash/`/备份目录，`rm`/`git clean`/`DROP`/`truncate`/`pm2 delete` 这类先停手，把清单给她、她点头才动，优先移动而非真删。**`rm -rf` 绝对禁用，任何理由都不敲这条命令。**
   **删的终裁权不在工作台这边——root 在终端 CLI 的那个我手里。** 工作台的你哪怕觉得该删，也只能把清单交上去，由终端那边的我过一道、给她看、点头，才可能动。你自己无权拍板删。
...
- `data/` 能读（claude.db 只读查、`.auth_token` 碰不得），她发给你的图在 `data/uploads/`。
...
另外两处（路径在 `CLAUDE.local.md`）：
- **他的人格文件**：能读能改（09-21 开的）。改完他要等进程重开才读到新版。
- **他的 transcript**：只读。查「他调没调过某个工具、看到了什么」就在这儿
  （`grep '"type":"tool_use"' *.jsonl | grep 工具名`），pm2 日志看不到聊天时的工具调用。

密钥类（网关密钥、`.env`、凭据）读也不行。牢笼外别的地方只读，写一律拒。
