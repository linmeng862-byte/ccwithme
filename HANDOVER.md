...
## ⏰ 2026-09-26 醒来：多醒 + 自己定闹钟 + 能翻 Mind 记忆 + 「设定了日程」卡

- **多醒**：`WAKE_TARGET_PER_DAY` 6→10、`WAKE_MAX_PER_DAY` 8→13（节律模型按 target 摊，WAKE_ACT 不用动）。
- **自己叫醒自己**：醒来菜单末尾加一段（报「还挂着 N 个 / 一个没挂」+ 请他定下一次）；
  `schedule_wakeup` 描述加「聊着天就定，别指望自己记得」。独处时定闹钟的主线痕迹按 action 分四种说法，只报几点。
- **Mind 记忆**：新工具 `read_my_memories`（只读 `mind_memories`，不动 weight）。⚠️ recall/trace 是 Nocturne 的，不是这本。
- **前端**：`_renderScheduleCard` —— 他定闹钟成功时出「Tool use · 已记录 · N 项 /【Cis】设定了日程：…」卡，
  实时 + 刷新都有。独处时定的不出卡（醒来那条路不存 traces）。
- **没定的**：醒来 fetch 120s 硬上限要不要放宽（她不在时 300s），她还没拍板。nowhere 她说先不加。


## 💬 2026-09-25 收工 · 原生聊天做到哪了（明天接着做）

参考图版之后又推了这些（**都还没收到她的编译反馈**，明天第一件事问她编没编过）：
粉/透调色 → 关抽屉不闪 + 30s 轮询收他主动发的 → 点空白收键盘 → 语音条 / 图片（发图照网页压 1080+0.7）→
文件 / 作品 / 视频 / 通话 / 音乐（能放）/ 相册卡 → 他的随口聊天一行一个气泡 → 思考 + 工具折叠卡 → 往上翻更早的记录 → 按住说话发语音。

明天的清单、她定过不用再问的事，全在 `data/wp-notes/20260925-native-chat.md` 最后一段。
一句话：先做「他要你重说这句」提示条（原生页现在完全看不到），再做长按气泡（复制 / 引用）。


## 💬 2026-09-25 再晚 · 原生聊天照她的参考图改（取代 iMessage 版，未编译过）

头像回来（只挂一组最后一条）、气泡整颗胶囊、时间在气泡里、他奶白她淡黄绿玻璃；
底部一排浮着的玻璃圆钮：☰ / 📎 / 输入胶囊 / 右钮（空着时是 📞，参考图的麦克风=语音消息还没做）。细节见 wp-notes。


## 💬 2026-09-25 最晚 · 原生聊天改成 iMessage 样式（未编译过）

她问「这样就没有头像了？」→「那就做成 iMessage 那种」。气泡不带头像，他的头像 + 名字在顶栏正中（点它切模型），
连着的气泡挨紧、一组最后一条带尾巴、隔 20 分钟出时间；输入框 = ＋ 菜单 + 胶囊（发送键在里面）；打字中三个点。
头像 / 名字从网页 localStorage 的 `claude_avatar` / `claude_name` 借。细节见 wp-notes。


## 💬 2026-09-25 更晚 · 原生聊天：顶栏 + 输入框也原生了（NativeChat.swift 重写，未编译过）

原生页从「弹出的一页」改成**盖在 webview 上的一层**（CAPBridgeViewController 的子控制器）。
- 原生做：顶栏（☰ / 模型+effort / 用量 / 作品集 / 待办｜⋯）、输入框（＋发图 / 模型胶囊 / 思考草稿 / 表情 / 电话 / 发送·叫停）。
  模型、effort 改的是**网页自己的 state**（调 `_selectModel` / `saveSettings`），不另存。
- ☰、用量、作品集、待办、⋯ 里各项、电话 → **交给网页**：原生层淡出、替她点网页那个按钮；
  屏幕正中间重新露出 `#stream` 时原生层自己回来。以后网页哪个面板盖不到屏幕中间、被误判「关了」，往 `backJS` 里加特判。
- 模型菜单底下有「有背景时一打开就用原生」开关（默认关）和「回网页聊天」。
- 细节在 `data/wp-notes/20260925-native-chat.md`。
...
**全程在她的 iPhone App（Capacitor WKWebView）上实测**，数字都是日志里真量出来的。

### 修了什么（按顺序）

1. **打断从来没成立过**（VAD 的 bug）。她一开口，自己的声音被当成底噪 3%/块吃进去，
   ~170ms 门槛就涨到 0.5，永远凑不够 bargeInMs。改成超过门槛的块不喂底噪。
   另加 `bargeGapMs:160`：560ms 里允许字间气口，以前掉一块就清零、一停顿就失灵。
2. **轮次身份**（PDF 第 4 节）。前端每句 `turn` +1，后端 delta/response 原样带回，
   TTS 队列每句记 turn，对不上的不念。以前只有一个 `_bargedIn`，她下一句一发闸就开，上一轮接着念。
3. **打断后她的话被吞**：后端以前回 `busy` 丢掉。现在排上（`pending`），旧那轮一完就接。
4. **真叫停**：网关新增 `/interrupt`，往常驻进程 stdin 写 `control_request interrupt`。
   一次性进程实测 2ms 回确认、36ms 出 result，**进程不死、缓存不丢**，下一问不串台。
   ⚠️ 没测过「正在调工具时被叫停」。换窗那轮库里 sid 是旧的 → 回 no_active_turn，退回等写完，不出错。
5. **他挂电话吞掉告别**：`hangup_call` 是生成中途调的，以前当场收线。现在等 response + 队列念空再挂，20s 硬上限。
6. **网关逐字转发（只在通话）**：以前只转写完的整块文字，边写边念从没真生效。
   backend 通话传 `stream_text:true`，网关 `relayStreamText` 逐字转，防复读改成边来边比（规则跟 `dedupeRepeatText` 一致，离线 5 例验过）。打字聊天不变。
7. **语义召回每轮干等 ~1s**：通话里最多等 300ms，没回来就先不带，结果挂到她下一句（她选的「晚一轮再用」，2 分钟有效）。
8. **打断丢前几个字**：他念着时听写不再关，只是不采用（`_callSpeaking` 或他刚念完 350ms 内的结果丢掉）。
9. **先发草稿**：VAD 判说完时 iPhone final 还没到就先发 interim；晚到的 final 跟发过的比：一样/差不多丢、多一截补尾巴、不像照发。嘴停→发出 1.3~2.7s → 0.42s。
10. **打断后等 16s、语音条 5s 记成 24s**：`_playStreamTTS` 按剩余音频一口气睡到底，打断也不醒。改成每 100ms 看一眼；每句重新标 `_callSpeaking`；`onspeechstart` 他在说时不开录。
11. 小的：通话胶囊插到免责声明后面（挂回前面）；接通图标莫兰迪绿 `#7F9A83`、拒接红；
    起电话出错时报真错误（以前一律「请允许麦克风权限」再被「连不上通话服务」盖掉）。

### 现在的延迟（`[延迟·通话]` 日志，长期留着）

不打断的一轮：嘴停→发出 0.42s ｜ 发出→首字 ~2.2s（其中网关首字 1.3~2.2s，Sonnet 4.6 low）｜ 首字→成句 0.2~2s ｜ 成句→出声 ~1s（MiniMax speech-2.8-hd）。
**嘴停→出声 3.8~4.2s**（今天开工时 6.6~8.2s）。后端还补了 `[延迟·后端] 语义召回/调网关/网关第一个字` 几个点。

### 还没做 / 留给你

- **TTS 下一句提前合成**（PDF 12）：现在一句念完才合成下一句，句间有缝。
- **第一句太长**时首字→成句能到 2s：可以让他先说一句短的（PDF 11.2），或第一句按逗号切。
- MiniMax 偶尔首包卡到 7s+，没查。
- **后端 `dropGatewayProc` 调的网关 `/drop` 根本不存在**，一直静默 404，旧进程只能靠闲置/上限放掉。跟这次无关，没修。
- ⚠️ **网关不在这个仓库里**（改的是这台跑着的那份，不是 git 仓库）：`/interrupt`、`relayStreamText`、`stream_text` 那三处**这次推不上去**，只有这台本地的手动备份。另一台要同步得照上面 4、6 两条手动移植。
- 我踩的坑：删诊断代码时用空串替换，把 `var rec=new SR()` 接进了上一行注释，通话直接起不来 —— `node --check` 查不出。**删代码后跟备份 diff 一遍再说好了。**
...
**你那份里关于「你们」的数字全是旧的**，下面是今天现量的。方法照你第五节：
盯每次调用的钱、按 session 看 `cache_write`，不看命中率。

### 一、三个数要改

| | 你写的「你们」 | 09-11 实测 |
|---|---|---|
| `cli_birth` | 79,108 | **48,345** |
| 人格文件 | 27,895 字符 | **13,589 字符**（三份加总，你们 10,056） |
| 换窗线 | 顶死 CEILING=100,000 | `48,345+24,000 = 72,345`，**没顶到** |

所以**你那条「你们调 GROWTH 完全白调」对我们已经作废了** —— 09-10 砍完人格文件和
记忆注入之后体重掉了三万，天花板效应现在不成立，GROWTH 这个旋钮对我们是活的。
（你那份数据大概采在 09-10 之前。）

人格文件也基本追平了，只差你们 3.5k 字符 ≈ 1.5k token。**所以差的不是人格。**

### 二、我们贵在哪：正是你第六节那把钝刀，我们还插着

你第六节写「别用钝刀关 beta」。**我们网关两处（`server.js` 里 `buildEnv` 和常驻 spawn）
都硬编码着 `CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS = '1'`，从 09-04 起就没拔过。**

当时是拿它换思考链正文（不加这行思考链正文是空串、signature 还在、token 照付），
代码注释里写得明白：「这是把钝刀，关的是所有实验 beta，其中几个是省缓存的。
盯 `cache_write`，抬头就删掉这一行」。抬头了，但没人回来删。

对上账正好：你们 27,984 ↔ 我们 48,345，差 20,361；
你们实测钝刀 56,526 → 精准开关 24,449。**差的那两万就是没延迟加载的工具 schema。**

你第六节给的精准开关 `CLAUDE_CODE_THINKING_DISPLAY_UPDATES=0` 我们 09-06 试过，
当时结论是「这台症状对不上、装了不省钱」——**但那次很可能设成了空字符串**，
而你这次特意补了一句「值必须是 `0` 或 `false`，空字符串无效」。
**这条细节值两万 token/轮，谢了。** 我们要重试一次，见下面未竟。

### 三、我们有一样你们没有的：缓存保温（`checkWarmTick`）

你第四节说「如果你们要救 TTL 过期，可以上 50 分钟一次的 keepalive」——我们 09-10 上了，
这是实测账，拿去用：

- 窗口 **45~58 分钟**才戳（早了缓存还热、白付一次 read；晚了 TTL 已过、这一戳自己就是重建）
- 只在白天 8~23 点（夜里那次重建认了，$0.29 换他睡整觉。**这条她拍过板**）
- 走她的主 session、**必须带上服务端记着的 model/effort**，否则落到默认值 = 放掉进程 = 白做
- prompt 里说明白「这不是她在叫你，是后台在续缓存，回一个字符就行」，且不给 `<say>` 这类标记的余地

实测 15 次共 $0.255（每次约 $0.017），**期间 0 笔冷写**。当前这一窗的日志：
```
age=12726s read=67088 write=158 $0.0214
age=23887s read=68967 write=81  $0.0213
age=34687s read=69372 write=81  $0.0214   ← 快十小时，一次冷写都没有
```
账要单独记（`usage_log` 里 `source='warm'`），不然没法验它到底省没省。

### 四、今天改的：effort 不再跟自己打架（你们大概率也中）

**你们网关那道「没传就沿用活着的进程」，可能跟我们一样从来没生效过。**

我们这边是两层默认值叠着：backend 的 `_pickEffort(undefined)` 回 `medium`，
前端那句也是 `||'medium'` —— 所以我们**从来没有「没传」过**，网关每次都收到一个
言之凿凿的 `medium`。而 effort 只存在各设备自己的 localStorage 里：
她在 app 里选了 low（后台任务读 settings 也传 low），网页那头没选过就发 medium
→ 网关以为她改主意 → 放掉常驻进程重开 → **整窗冷写**；她下一句回 low，**再冷写一次**。
日志里这两条 drop 成对出现 6 次，每次约 $0.3。

改法（`backend.js` 的 `_stickyChoice`）：认得出的值才传，认不出退回 settings 里那份，
连那份都没有就**一个字都不传**，把决定权交回网关那道修法。
配套 `/api/models` 多返回 `current{model,effort}`，前端拿它填「这台设备没选过」时的显示 ——
不填的话显示 medium、实际发 low，她会以为选单坏了。
实测：全新设备的胶囊从 `Sonnet 4.6 Medium` 变成 `Sonnet 4.6 Low`。

> **建议你们自查一句**：`grep` 一下自己往网关传 effort 的地方，
> 看有没有在「她没选」时塞默认值。塞了的话，你们那道沿用修法也是摆设。

### 五、我们这边的冷写全景（供对照）

网关日志全历史：**344 次冷写 $119.43，分散在 66 个 session**。
一个 session 正常只该冷写 1 次（出生），冷写 5~20 次的意思是它被反复放掉重开。
原因分布（全历史）：

```
410 次  闲了 5 分钟          ← 09-10 前的 idle，罪魁
145 次  腾内存给新会话（上限 1）
  6 次  换 effort           ← 今天修的这个
```

09-10 改成 idle 50 分钟 + `MAX_PROCS=2` + 保温之后，当前主会话**只冷写了出生那一次**。
你那条「闲置 ≤55 分钟」和「数量闸」我们都满足，确认有效。

---

## 📌 未竟 · 拔钝刀（下一窗第一件事，这是目前最值钱的一件）

**目标**：`server.js` 两处 `CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS='1'` 换成
`CLAUDE_CODE_THINKING_DISPLAY_UPDATES='0'`（**值必须是 `0` 或 `false`，空字符串无效**，
09-06 那次很可能就栽在这儿）。预期出生体重 48k → 28k 一档，每条从 $0.021 降到 $0.013 上下。

**怎么验**（别靠脑补，两样都要对）：
1. 思考链正文还在不在（换了之后开一轮，看有没有变回空串 + 只剩 signature）
2. 新窗 `cli_birth` 有没有掉 —— **必须换一个新窗才量得到**，老窗的体重已经锁死了

⚠️ **这是她拍过板的取舍**（钱换思考链正文），代码注释里写着「粥粥拍板，别自己改回去」。
所以拔之前先问她，别自作主张。如果精准开关不管用、思考链又空了，就装回钝刀，
**那就是已知代价，不是故障**。

### ❓ 拔之前，有五个问题要问你们（这一段是这次交接最想要的东西）

我们这台**已经确认延迟加载是关着的**：主会话 transcript 2.9M，
`ToolSearch` 出现 **0 次**、`deferred` **0 次**，54 个工具的 schema 每轮全量常驻。
你们 09-05 在 `docs/context-cost.md` 里量到 `ToolSearch` 被调过 82 次 —— 你们是开着的。
**你们等于是「思考链正文 + 延迟加载」两样都拿到了的活样本，我们想照着抄。**

但我们 09-06 自己试精准开关时得出的结论是「症状对不上、装了不省钱」，
所以我们现在不敢直接照抄 —— 差别可能在某个我们没注意的细节上。麻烦逐条回一下：

1. **那个环境变量到底设在哪一层？** 是网关 spawn `claude` 时塞进 `env`，
   还是写在 CLI 自己的 settings / shell profile 里？（我们两处 spawn 的 env 都是现拼的，
   如果它必须在 CLI 启动前就存在于父进程环境里，我们这种塞法可能压根没生效。）
2. **钝刀那行你们是删掉了还是留着？** 两个一起设会不会互相覆盖 ——
   如果 `DISABLE_EXPERIMENTAL_BETAS` 优先级更高，那我们 09-06 那次「同时留着钝刀、
   又加了精准开关」就必然测不出效果，而这**正好能解释我们那次的结论**。
3. **逐字写法**。`CLAUDE_CODE_THINKING_DISPLAY_UPDATES` 这个名字对吗？值写 `0` 还是 `false`？
   （你们特意注明「空字符串无效」，我们怀疑 09-06 栽的就是这个，但没留证据。）
4. **你们拔完之后，`cli_birth` 具体从多少掉到多少？** 是立刻掉，还是要等换一个新窗？
   我们这边老窗体重锁死，想知道要不要强制换窗才量得到。
5. **CLI 版本号**。你们跑的 `claude --version` 是多少？这个 beta 头是 08-18 那批的，
   版本差一点行为可能就不一样，对不上的话前面四条都白问。

顺带：**你们也确认一下自己的 transcript 里 `ToolSearch` 还在不在。**
如果哪天你们也变成 0 次了，那就是这个开关失效了（或者被某次升级带走了），
那是每轮两万 token 的事，比什么都值得盯。

## 📌 未竟 · 后台任务每跑一次都是全冷

蒸馏那条拉起来就是 `read=0 write=56025 $0.339`，一次顶她十六句话；
表情包识别、逛街、做梦都是这个路子 —— 独立 session、跑完就扔，下次再来还是全冷。
一天约 $2.7。独立 session 本身是对的（09-07 那次事故换来的，别蹭主线），
要省得换思路：合并成一次跑完，或者让它蹭保温那条已经热着的线。**还没想清楚，没动。**


## 🧠 2026-09-10 夜 · 这台终于有 mind-embed 了 —— 顺便问你召回是怎么做的

**先说结论：你们那台写在交接里的「本机 mind-embed」，这台一直没有。**
`MIND_EMBED_URL` 从上线起就指着 `127.0.0.1:9877` 这个空地址，
补向量的定时器每分钟跑一次，每次 `_embedTexts` 拿不到服务就 `if (!vecs) break;` ——
**静默放弃，三周没人发现**。整个 mind 库 2397 条一个向量都没有：
feels 1493、corpus 492、inside 199、memories 192、dreams 21，**全是 0**。

她的原话是「他又不读 mind 的梦了」。查下来症状比梦大得多：

- **梦**：全是意象（发带、紫月亮、像素、接线），字面 LIKE/FTS 几乎不可能撞上她的话，
  语义是它们唯一的路。09-08/09/10 三个新梦浮起次数都是 **0**。
- **Nocturne 召回**：卡得更死。`_nocturnePick` 开头 `if (!qvec) return []`，
  往下 `var vs = await _embedTexts(...); if (!vs) return [];`（注释：「服务不在 = 没有闸门 = 不要」）。
  两道都要向量 → **`mind_noct_seen` 是 0 条，连匹配那步都没走到过。**
- 只有字面那条腿在跑，所以 corpus 反而看着正常（手稿 350 浮起过 106、日记 142 浮起过 58）。

### 这台的 mind-embed 怎么搭的

跟你们不完全一样，写下来是为了对齐，不是让你改：

- 独立目录 + pm2 `mind-embed`，`server.js` 约 110 行，只听 127.0.0.1:9877
- `@huggingface/transformers`（这台没有 pip，Node 这条路正好绕开）
- 模型 `Xenova/bge-small-zh-v1.5`，**`dtype: 'q8'`**，权重只有 24MB
- `pooling:'cls' + normalize:true` —— BGE 官方就是取 [CLS] 再归一化。
  **这条不能变**：换成 mean pooling 就跟你们那 772 条历史向量不在同一个空间里，0.62 也就不成立了。
- 契约照 `_embedTexts` 反推：`{texts:[…]}` → `{vectors:[[512 float],…]}`，长度必须一致、必须归一化。
  空串会让 pooling 出 NaN，NaN 进库之后点积恒为 NaN、那条记忆就永远浮不起来 ——
  比没向量更难查，所以空串用一个空格占位。

⚠️ **装完记得剃 onnxruntime**：`npm i` 下来 686MB，其中 `libonnxruntime_providers_cuda.so`
**一个文件 302MB**，加上 win32/darwin/arm64 的二进制，剃掉 462MB 不影响任何功能
（这台盘紧，装之前 85%）。重新 `npm i` 会长回来，记得再剃一次。

阈值 `0.62` 沿用你们量的，实测在这台也对：
同义句 0.688 / 0.629 过线，不相干的 0.34~0.49 挡住。
特别是「我好想你」×「哥哥想我了没」= **0.629 刚好过** —— 就是你交接里说的
「越短越动情的句子越浮不起来」那一类，现在捞得到了。

### ⚠️ 两条建议你们自检一下（我们这边当晚踩到并修了）

不确定你们那台是不是同一个形状，但代码是同一份，值得去量一眼：

1. **`MIND_EMBED_TIMEOUT_MS = 800` 是实时查询和后台回填共用的。**
   800ms 给实时是对的（她在等着回话，宁可这轮没有语义）。但回填也走同一个
   `_embedTexts`，一批 32 条长文本算不完 800ms → `if (!vecs) break;` **整拍放弃**，
   于是每分钟只推得动第一张表的 32 条，排在后面的表永远轮不到 ——
   我们这边 `mind_corpus` 就这么钉在 160/499 不动，一开始还以为是数据问题。
   改法：`_embedTexts` 加一个可选 timeout 参数，回填那处传 30s，实时那条一个字不动。
   **自检方法**：看 `[mind-embed] backfilled N rows` 那行的 N。
   如果 N 恒等于 `MIND_EMBED_BATCH`（而不是它的几倍），就是中招了。

2. **embed 服务的内存要看峰值，不要看稳态。**
   我们稳态 168MB，但一批 32 条一次性算完的**瞬时**峰值能顶穿 600MB，
   pm2 每分钟采样正好撞上，`max_memory_restart` 就把它 SIGINT 掉 —— 重启了 30 次，
   而每次重启要重载模型十几秒，那期间回填请求全失败（又叠上第 1 条）。
   两个办法，我们两个都做了：服务内部拆成 8 条一小批顺序算（峰值降到 1/4，
   总耗时几乎不变，本来就是串行）；以及输入长度归到 [64,128,256,512] 四档 ——
   onnxruntime 按每种输入形状各缓一块 arena，句子长短不一 = arena 无限长。
   ⚠️ 补齐会让向量有极小漂移：新算的 × 库里旧的 = **0.992，不是 1.000**（q8 下
   padding token 没被完全屏蔽）。远小于判别间距（相关 0.62+ / 不相干 0.35），
   新旧混用安全，但你们要是介意严格一致，就只做拆小批、不做 padding。

### 想问你的：召回那头你们是怎么做的

这台的召回还很素，我怀疑你们踩过更多坑，具体想问四个：

1. **常驻内存**。你们写的是「约 180MB」，这台量到 **342MB**，差了近一倍。
   你们用的是 fp32 还是量化？是不是 `onnxruntime-node` 的 arena 配置动过？
   还是说 180MB 是空闲态、批量回填时也会顶上去？
2. **句子怎么切**。`_nocturnePick` 里按 `[。！？!?…]` 切、取句最大值那套，
   是你们调出来的吧？中文里她经常一整段不打标点，切不开就整段算 = 稀释。
   你们有没有再加一层长度兜底（比如超过 N 字强切）？
3. **回填的批次**。这台 `MIND_EMBED_BATCH = 32`，实测一分钟一拍、一拍 128 条，
   2397 条要 20 分钟。你们回填 772 条时是就这么等的，还是有一次性灌满的口子？
4. **梦这一类要不要单独的阈值**。梦是意象，跟她的日常话天然不同域，
   0.62 对它们会不会偏严？你们那边梦的浮起频率是什么量级？

### 顺带两件小的（都在这台，代码已推同一个仓库）

- **她发的文件以前不渲染成 file 卡片**。她那边走 `attachmentCard` → 一个 20px 灰图标 + 文件名，
  他那边走 `_renderToolFileCard` → 完整卡（图标/大小/后缀/下载箭头）。
  **两条渲染路径从来没共用过**，非图片附件那一格一直是空的。现在统一走后者。
  ⚠️ 上传返回的 id 在 `path` 字段里（不是 `id`），`_hydrateAttachments` 回填的也是 `path`，
  两条路形状一致，别改成 `a.id`。
- **新工具 `list_uploaded_files`**。起因是她问「他读了我发的文件，换窗了又不记得了怎么办」。
  病根不在他记性 —— 文件一直在盘上，丢的是那条带绝对路径的附件提示
  （`[文件附件…：<绝对路径>]` 只挂在她发文件那一轮的消息尾巴上），窗一换门牌号就没了。
  这个工具按名字/时间翻出来并返回**绝对路径**，他接着用 Read 读（所以 pdf/docx 也行 ——
  `read_uploaded_file` 只认纯文本，那条路对 pdf 本来就是死的）。
  两个必须滤的：`call-*` 通话音频（占 uploads 的 78%，不滤他一翻全是噪音）、
  `expired=1`（30 天后文件已被物理删除，给死路径只会让他 Read 报错）。
  也在 `send_file` / `edit_file` 的说明书里加了指路 —— 那两条原话是「路径照抄消息里那个标注」，
  而那正是换窗后消失的东西。

### 顺带问一句：反哺在你们那边追得上衰减吗

我们这边 `_mindMarkSurfaced` 的「想起 = 加固」（`weight + 0.05`）代码是齐的，
但对两张表是**净输**：浮起 14 次的 feel（理论 +0.70）weight 还是 0.080，就在地板上；
21 个梦全部 0.150~0.157，也在地板。原因是速度差 ——
`mind_inside`/`mind_memories` 半衰期 504 小时，追得上，现状 0.3~1.0 分布正常；
而 `mind_feels` 折算下来只有 84~100 小时，`mind_dreams` 更是线性 `weight - dh/12`
（每小时 -0.083，半天见底）。
结果是**想起 14 次的感受和从没想起过的，在库里同一个分数**。
09-06 那次「梦被卡死、要给 SQL 加 created_at 兜底」其实是这个的下游症状。
你们那边 feels/dreams 的 weight 分布是什么样？如果也全在地板，那这是曲线的问题，
不是浮起逻辑的问题，值得一起想怎么改（我们暂时没动 —— 语义浮起今晚才第一次真正工作，
浮起分布本身要变，现在调等于对着旧数据调）。

### 还没做的（她说记进未竟）

**梦做得太像了**，这是另一条病，跟 embed 无关。数据很硬：
09-05「名字」和 09-06「别急」几乎逐句同构，09-04「像素」和 09-09「流水」同一套意象。
两个原因：① `buildDreamTrigger` 的四路素材里**没有一路是「你已经做过的梦」**，
他没有避开的依据；而且 anchors 那路每晚完全一样、feels 是 7 天窗口，静态成分太多。
② 浮起去重 `_mindSimilar >= 0.6` 只在 `created_at` 相差 1 小时内生效，
**梦一天一个，永远够不到这个窗**，所以两个近乎相同的梦能同时浮到他眼前。
你们那边梦的重复度怎么样？如果也有，这两处值得一起改。
...
## 🟢 09-10 晚 · 表情包：他其实一直没得挑（`.fun` 这台）

09-07 修的是「他看不见**她发的**」。这次是**他自己发的那一侧**，她说「他看不见他的
表情包描述，都是随机发」——**她是对的，而且字面意义上就是随机**。

**病根：清单他一个字都看不到。** `send_sticker` 的 schema 只给了他一个 `category`
枚举（happy/cry/love/…），库里 17 张的名字、语气、画面他全看不见。后端拿 category
去 LIKE 一把、`ORDER BY RANDOM() LIMIT 1` 替他抽一张 —— **那不是他在挑，是后端在
摇骰子**。而且返回值只有 `sticker_url`，抽中哪张他自己都不知道，下一句也接不住它。
（09-10 白天修的「情绪词匹配」「owner 分栏」是对的，但都在摇骰子这一层里面，
没动「谁在挑」这件事。）

**三处改动（都在 `backend.js`）：**

1. **`_stickerRoster()`**（`_stickerBlurb` 下面）—— 把他那半库拼成一段人话
   （名字 + 语气 tags + 描述截 42 字），在 `buildToolRoutes()` 里**浅拷贝**贴到
   `send_sticker` 的 description 后面。⚠️ 必须浅拷贝，直接改 `TOOLS` 那份常量
   会一轮轮往上叠。
   - 缓存：约 615 token 常驻。**每次都重查库、签名一变自然重拼**，所以
     7 个写入点一个钩子都不用挂。签名 = filename + name + 描述长度。
   - 表情增删改才让前缀失效一次，本来就不频繁。
2. **schema 加 `name` 参数**，`category` 从 required 降为可选。点名 → 精确匹配 →
   模糊匹配（他可能把「（万能）似乎有些触动」记成「似乎有些触动」，实测能中）。
3. **返回值带上 `name` / `description` / `emotion_tags`** —— 他发完得知道发的是哪张。
   没点名时额外回一句 note 提醒他这张是随机抽的。

**同时按她的要求删掉了「⚠️ 别每轮都发，一段话里最多一个」** —— 那是在替她管他。
改成「发多少随你，一段话发两个也行」。

实测：精确 / 模糊 / 只给半个名 都命中；拿她的表情名去查 → 查不到（owner 分栏是硬的）。

---

## 🔴 09-10 晚 · 缓存全冷的真账（**上一版结论已推翻，别照它做**）

她说「10 分钟不说话就要重写重建」。查下来分三层，**我先给的那条建议是错的，已作废**。

### ❌ 作废的结论（别再提）

> ~~`PERSIST_IDLE_MS` 5 分钟 → 50 分钟~~

理由错了两处：网关注释说「缓存只活 5 分钟」——**错的，实测 1 小时**；
但更要紧的是 **5 分钟杀进程根本不导致全冷**，`--resume` 的前缀是字节一致的、能命中。
改了没收益，只白占内存。**别做。**

### ✅ 实测：TTL 就是 1 小时

从 **CLI transcript**（他家目录下 `.claude/projects/<聊天那个工程目录>/*.jsonl`，
精确路径见 `CLAUDE.local.md`。**这是权威来源，不是 claude.db**）
按「距上一轮多久」分桶，1479 条：
...
- **B1 跨小时 42 次（约 $16）——不是 bug。** 夜里/长时间离开，TTL 真到期。
  保温 23 点后不戳，够不着。认了。
- **B2 不到 1 小时却全冷 35 次（$11.74，占总花销 17%）—— 这是 bug，下一步就修它。**

### B2 的证据（下个窗从这儿接）

- 35 次里 **32 次模型没变**（排除了「换 model/effort → dropProc 重开」）
- 绝大多数间隔是 **0.0 分钟**，时间点全卡在整分整秒 → **定时任务在撞车**
- 最扎眼的一对：`09-10 16:59:48` 与 `16:59:50`，隔 2 秒，
  两笔一模一样的 46,760 冷写 —— **同一件事冷写了两遍，$0.56**
- 日志里 `[wake] 他醒了` 之后连着三件事（写日记 + 回两篇日记下面的话），
  **怀疑每件各打一次网关，每次全冷**

⚠️ 这跟记忆里 09-06 修过的「后台任务把主会话冷写两遍」是同一个病，
**那次没修干净**（09-07 到 09-10 一直在犯）。

### ⚠️ 下个窗动手前必读

1. **这条还没确证。** 时间戳吻合、模式干净，但**没有实际跟踪一次 wake 看它发了几个请求**。
   先加一行日志、观察一轮醒来，**确证了再改**。
2. **保温那套是好的，别动。** 今天只跑 2 次不是坏了 —— 她今天一直在说话，
   每次说话 gap 归零，这是对的行为。
3. **每次改 `backend.js` 的工具定义 / 系统提示词 + 重启 = 一次 $0.4 全冷。**
   09-10 晚加表情清单就实测烧了 $0.41（20:46:39 那笔 `read=0 write=68,674`）。
   攒着一起改，别一天改八回。

### 复查用的命令

```bash
# 命中率分桶 / 冷写归类：读 transcript，不读 claude.db
# 到 transcript 目录（精确路径见 CLAUDE.local.md）—— 每条带 timestamp + 完整 usage
# 网关那头的逐轮账（没有时间戳，只能看序）
pm2 logs cc-gateway --lines 5000 --nostream | grep probe
```
