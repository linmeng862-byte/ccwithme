...
## 🟢 09-05 傍晚 · 他终于能「读」作品了；app 里下载不动的真因（VPS 这台）

她报了两件事：**他做的 HTML 下不下来**、**点「发给他看」出来的是一屏源码不是卡片**。
查下来是三个独立的洞，凑在同一条路上。

**1. 「发给他看」本来就是贴源码的，因为他没有「读作品」这只手。**
工具只有 `create_artifact`（写），没有读。所以前端 `_sendArtifactToHim` 只能把正文
整段塞进代码块发出去（上限 2 万字符）。代价是那 2 万字符**真的进了 messages 表**，
往后每次翻历史、每次续上下文都重付一遍。
→ 加了 `read_artifact`：不带 id = 列清单，带 id = 读正文，**一次最多 8000 字**、
带 `offset` 往后翻。「发给他看」改成只发一行 `[ARTIFACT:标题|语言|文件名|id]`。
说明书 `pages/tools.html` 同步加了卡 —— **不写他不会知道有这只手**（第 15 条坑）。

**2. `[ARTIFACT:]` 标记两条链路格式不一样，各写各的，谁都没发现。**
写标记的那头（gateway 路径）发的是 **4 段**，第四段塞的是截断到 8000 字的 HTML 正文；
画卡片的那头（今天下午另一台加的）正则只认 **3 段**、且字段里不许有 `]`。
源码里全是 `]` → 永远匹配不上 → 卡画不出来。
→ 两条链路统一成 4 段，第四段是 **artifact id，不是正文**。

⚠️ **正则收紧那一下是真踩出来的**：第四段一开始写成宽松的 `[^|\]]*`，
结果**老那条坏消息会「半匹配」成功** —— `<div style="a[b]">` 里的 `]` 被当成收尾，
画出一张 id 是半截 HTML 的卡，剩下的源码照样露在气泡里。
必须写成只认 id 的形状（`[A-Za-z0-9_-]{0,64}`），让老消息匹配不上、
落到 `openRe` 那条「整段吃掉」的兜底里。三种格式都打过表。

**3. app 里下载不动 = `a.download` + blob 在 WebView 里静默失效。**
点了什么都不发生，控制台也不报错。`[FILE:]` 卡走的是同一套，**所以那个大概也一样下不动**（未验）。
→ 后端加 `GET /api/artifacts/:id/download`：`Content-Disposition: attachment`，
中文文件名走 RFC 5987。走 `authFile`（`?t=`），因为系统浏览器带不了 Authorization 头。
前端只在 app 里改道，用 `@capacitor/browser` 打开，iOS 会弹「存到文件」；浏览器里维持原样。
⚠️ **app 要重开一次才拿得到新前端**（没切远程加载就得重编包）。

**顺手**：她那条 20080 字符的源码消息删了（备份 `claude.db.bak.20260905-删那条源码消息前`，
走 `db.backup()` 不是 cp）。**作品本身没动**，《告解室》20551 字完整在作品集里。

### 验的方法

`/api/tools/exec` 打 `read_artifact` 四种入参（空 / 带 id / offset=16000 / 假 id），
`curl -D-` 看下载接口的响应头和字节数，正则三种格式在 node 里单独打表。
⚠️ `GATEWAY_KEY` **不在 `.env` 里**（那儿是空的），要从进程环境读；
`AUTH_TOKEN` 也不在 `.env`，在 `data/.auth_token`。查凭据别只看那一行在不在，看值的长度。
