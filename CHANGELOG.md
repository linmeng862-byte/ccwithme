...
#### 关窗轮数 `CLI_ROTATE_AFTER` 48 → 96
她说「48 轮关窗会不会太快，感觉还没聊什么就关窗了」。查 `usage_log` 实测，
**代码里那条「`--resume` 每轮把全部历史重新写进 prompt 缓存」的假设是错的** ——
缓存实际是复用的，稳态每轮 `cache_write` 只有 160~660，`cache_read` 每轮才涨 200 左右：

| 轮次 | cache_write | cache_read | 本轮 |
|---|---|---|---|
| 117（换会话） | **49237** | 0 | **$0.1976** |
| 128 | 237 | 32440 | $0.0083 |
| 133 | 187 | 33712 | $0.0081 |

真正贵的是**换会话那一次**，比平常贵 24 倍。所以逻辑是反的：**换得越勤越贵。**
按尖峰摊薄算每轮均摊：48→$0.0116 / **96→$0.0109** / 144→$0.0117，最优在 96 附近。
48 轮时上下文才 3.4 万 token，96 轮约 4 万，离 autocompact 触发线（十几万）还远。
原处那条过时假设已就地标注证伪，免得后来人照着它推理。

⚠️ 人格不受影响：每开新会话都重读人格文件，他是从同一份文件重建的。换窗丢的只是
对话细节，那部分有 `recentRecap`（最近 8 条）+ Nocturne 记忆浮现 + `search_chat_history` 接着。

### 🐛 修复

#### 分片消息里的卡片渲染不出来（承上一个 commit）
`_splitMessages` 拆出来的 clone 只调了 `renderMessage`，没调 `_renderVoiceCards` /
`_renderCmdCapsulesAfter` —— 所以 `[VOICE:]` 一旦被分到某一片上就渲染成裸标记。
加了 `_reRenderCards()`，每一片（含复用的第一片）都补走一遍卡片渲染。

上一个 commit 里「带方括号标记的整条都不拆」是**过度修复，已撤销** ——
他常发语音条，那等于大半消息都不分片了。正解是拆归拆，每片补渲染。

#### clone 继承了原 row 已渲染的胶囊
`row.cloneNode(true)` 是深拷贝，把已经渲染好的 `.cmd-capsule` 一起复制过去了
（胶囊挂在 `.msg-body` 上，而 `renderMessage` 只重写 `.md`，清不掉它）。
clone 后先清掉继承来的胶囊，让每片按自己的内容重新渲染。
